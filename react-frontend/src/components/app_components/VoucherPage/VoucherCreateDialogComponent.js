...
const VoucherCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.termandcondition)) {
                error["termandcondition"] = `Termandcondition field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            id: _entity?.id,categoryid: _entity?.categoryid,points: _entity?.points,termandcondition: _entity?.termandcondition,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                <label htmlFor="termandcondition">Termandcondition:</label>
                <InputTextarea id="termandcondition" rows={5} cols={30} value={_entity?.termandcondition} onChange={ (e) => setValByKey("termandcondition", e.target.value)} autoResize  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["termandcondition"]) ? (
              <p className="m-0" key="error-termandcondition">
