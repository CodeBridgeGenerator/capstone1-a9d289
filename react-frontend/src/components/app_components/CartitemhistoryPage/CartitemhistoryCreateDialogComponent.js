...
const CartitemhistoryCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            id: _entity?.id,voucherid: _entity?.voucherid,userid: _entity?.userid,quantity: _entity?.quantity,completedate: _entity?.completedate,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                <label htmlFor="completedate">Completedate:</label>
                <Calendar id="completedate" value={_entity?.completedate ? new Date(_entity?.completedate) : null} onChange={ (e) => setValByKey("completedate", e.value)} showTime hourFormat="12"  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["completedate"]) ? (
              <p className="m-0" key="error-completedate">
                {error["completedate"]}
              </p>
            ) : null}
          </small>
